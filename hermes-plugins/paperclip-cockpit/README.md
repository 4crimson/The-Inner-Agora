...
Projects may keep their own wrapper command for compatibility. Live mutation commands such as `run`, `retest`, `cleanup`, and `notify` require explicit `--live-ok`; planning, readiness, dry-run, profile-plugin-sync, guard-repeat, release-live-gate, summary, report, acceptance, release-gate, and bug-batch commands are non-live. `profile-plugin-sync` compares the repo plugin tree with the installed Hermes profile plugin tree; `guard-repeat` reruns the configured health guard after manual repair and records `repairBackup`, `repairCommand`, and `guardRepeat` in the manifest; `release-live-gate` writes the top-level release evidence wrapper and delegates to release-gate/evidence-checklist; `release-gate` does not run live suites and instead aggregates existing run manifests plus backup/profile-sync/repair evidence into a release decision artifact.
