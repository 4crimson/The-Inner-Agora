...
node hermes-plugins/paperclip-cockpit/qa-tool/bin/paperclip-qa.mjs release-live-gate --config telegram-testing.config.json --suite service-commands --run QA-... --backup-id BACKUP_ID --profile-plugin-sync ok --commit COMMIT --json
node hermes-plugins/paperclip-cockpit/qa-tool/bin/paperclip-qa.mjs release-live-gate --config telegram-testing.config.json --suite service-commands --commit COMMIT --live-ok --json
...
Projects may keep their own wrapper command for compatibility. Live mutation commands such as `run`, `retest`, `cleanup`, `notify`, and `release-live-gate --live-ok` require explicit `--live-ok`; planning, readiness, dry-run, profile-plugin-sync, guard-repeat, release-live-gate without `--live-ok`, summary, report, acceptance, release-gate, and bug-batch commands are non-live. `profile-plugin-sync` compares the repo plugin tree with the installed Hermes profile plugin tree; `guard-repeat` reruns the configured health guard after manual repair and records `repairBackup`, `repairCommand`, and `guardRepeat` in the manifest; `release-live-gate` writes the top-level release evidence wrapper and, with `--live-ok`, also creates a read-only Paperclip backup, runs one suite, cleanup, configured guards, release-gate, and evidence-checklist; `release-gate` does not run live suites and instead aggregates existing run manifests plus backup/profile-sync/repair evidence into a release decision artifact.
