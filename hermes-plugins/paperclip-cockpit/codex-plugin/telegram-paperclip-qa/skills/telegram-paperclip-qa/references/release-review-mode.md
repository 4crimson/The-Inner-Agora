...
8. Run `release-live-gate --config telegram-testing.config.json --suite SUITE --run RUN_ID --backup-id BACKUP_ID --profile-plugin-sync ok --commit COMMIT --json` to write the top-level release evidence wrapper.
