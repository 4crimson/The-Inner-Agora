#!/usr/bin/env python3
"""Telegram userbot E2E driver for Paperclip Cockpit QA.
...
async def send_message(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        if not await client.is_user_authorized():
            raise SystemExit("Userbot is not authorized. Run the qa-tool telegram userbot driver with: login")
...
async def notify_message(args: argparse.Namespace) -> dict[str, Any]:
    missing = validate_config(require_phone=False)
    if missing:
        raise SystemExit(f"Missing required env: {', '.join(missing)}")
    cfg = config()
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Telegram userbot E2E driver for Paperclip Cockpit QA")
    ...
    notify = sub.add_parser("notify", help="Send one notification message without reading replies")
    notify.add_argument("text")
    notify.add_argument("--dry-run", action="store_true")
    notify.set_defaults(func=lambda args: asyncio.run(notify_message(args)))
