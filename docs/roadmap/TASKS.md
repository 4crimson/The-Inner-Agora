...
**Статус на 2026-07-02:** реализован Phase 4 MVP: T4.0–T4.6, T4.9, T4.10, T4.11 и явная-marker часть T4.13. Evidence: `docs/superpowers/specs/2026-07-02-phase-4-human-assistant-design.md`, `docs/superpowers/plans/2026-07-02-phase-4-human-assistant.md`, `scripts/intent-slots.mjs fixture-20 --routing-mode regex --json` = 20/20, `ROUTING_MODE=llm INNER_AGORA_LLM_MODEL=gemma-4-26b-a4b-it-mlx node scripts/intent-slots.mjs fixture-20 --json` = 20/20 with regex guardrail fallback on 2/20 phrases, avg 1778 ms. Остались P1/P2 хвосты: полноценный `/start`-онбординг (T4.7), пошаговый wizard UI (T4.8), `dialogueWithContext()` с выжимкой синтеза (T4.12), и более мягкая эвристика T4.13 для реплик без явного follow-up marker.
