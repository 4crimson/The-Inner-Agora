...
| Live Telegram/Paperclip | Hermes profile `inneragora`, Paperclip company/project, Telegram bot/userbot | Approved for separate live validation as of 2026-07-05; `help`, `service-commands`, and strengthened `mode-routing` suites accepted after recovery/profile sync; post-suite guard required repair once and passed after the latest UX retest | Use explicit live QA commands and `--live-ok`; run guard after work-creating suites |
