...
| Live Telegram/Paperclip | Hermes profile `inneragora`, Paperclip company/project, Telegram bot/userbot | Approved for separate live validation as of 2026-07-05; `help`, `service-commands`, and `mode-routing` suites accepted after recovery; post-suite guard required a second `prepare local` | Use explicit live QA commands and `--live-ok`; launch-summary leakage remains a separate Telegram UX bug; run guard after work-creating suites |
